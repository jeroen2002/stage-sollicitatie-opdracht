    ...
    <div>
        ...
        <b-list-group>
            <b-list-group-item v-for="(value, index) in todos" :key="index">
                <b-row>
                    <b-col>
                        <input @click="toggleTodoStatus(value)" :checked="value.is_done" type="checkbox" id="todo" name="todo" value="todo" />
                        <label for="todo" :data-content="value.title" class="todo">{{ value.title }}</label>
                    </b-col>
                    <b-col style="top: 5px;">
                        <Show :todo="value" />
                        <Edit :todo="value" />
...
export default {
    ...
    mounted() {
        ...
        .then(res => {
            if (res.data.data) {
                this.$store.dispatch('getTodos', res.data.data);
            }
        });
    },
    updated() {
    ...
    methods: {
        toggleTodoStatus(todo) {
            axios.post(`/api/${todo.id}/toggle-todo`)
                .then(response => {
